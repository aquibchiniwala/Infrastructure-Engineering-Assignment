...
void move_Files_On_Desktop()        //create folders on desktop with top 10 file extensions and move files there accordingly.
{
    char desktop[50]={'\0'};
    #ifdef __linux__    //get Desktop path of current user in LINUX/UNIX.
        {
          strcpy(desktop,getenv("HOME"));
          strcat(desktop,"/Desktop/");
        }
       #else            //get Desktop path of current user in Windows.
        {
         strcpy(desktop,getenv("USERPROFILE"));
         strcat(desktop,"\\Desktop\\");
        }
    #endif
    int i=0;
